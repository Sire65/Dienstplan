...
(async()=>{
  ...
  assert.strictEqual(service.auditReady,service.validation,'Audit und Planner-Validierung müssen denselben Freigabestatus liefern');assert.ok(service.total>=0,'Audit muss Dienstanzahl liefern');assert.strictEqual(service.compressed.length,2,'Zusammenhängende Lücken gleicher Zone müssen komprimiert werden');const frontGap=service.compressed.find(x=>x.zone==='front');assert.ok(frontGap,'Komprimierte Vorne-Lücke fehlt');assert.strictEqual(frontGap.start,12);assert.strictEqual(frontGap.end,12.5);

...
ok(/^0\.19\.45$/.test(current.version),'current release is V0.19.45');
ok(current.releasePath==='release/v0.19.45/site','current.json points to canonical V0.19.45 tree');
...
ok(rules.includes('V0.19.45 Produktionsstand'),'rulebook is consolidated for current production generation');
...
for(const p of ['index.html','service-worker.js','src/core/update-manager.js','src/adapters/push.js','src/ui/push-center.js','src/adapters/diagnostics.js','src/ui/diagnostics-center.js','src/core/table-core-adapter.js','src/ui/kc-ux-polish.js','src/ui/kc-ux-polish.css'])ok(exists(p),'required runtime module exists: '+p);
const index=read('index.html');ok(index.includes('KC DP2 V0.19.45'),'visible release title is current');ok(!index.includes('KC DP V0.17.10 – kompakte Plansteuerung bereit.'),'obsolete visible status text removed');
...
const push=read('src/adapters/push.js');for(const m of ['reconcileExisting','ensureSession','sendSelfTest'])ok(push.includes(m),'push invariant present: '+m);ok(!push.includes('kc-dp-self-push-test'),'no floating self-test overlay in release');
...
const ux=read('src/ui/kc-ux-polish.js');ok(ux.includes('0.19.45'),'KC UX polish runtime identifies V0.19.45');

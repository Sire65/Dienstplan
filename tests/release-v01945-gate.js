...
const current=JSON.parse(fs.readFileSync(path.join(__dirname,'..','release','current.json'),'utf8'));if(current.version!=='0.19.45'||current.releasePath!=='release/v0.19.45/site')fail('current.json nicht V0.19.45');
...
console.log('KC DP2 V0.19.45 canonical release gate: PASS',manifest.files.length,'files',total,'bytes');

...
(async()=>{
  ...
  await page.waitForSelector('#sourceHealthCard',{timeout:20000});
  ...
  await page.waitForFunction(()=>document.querySelector('#sourceHealthCard')?.dataset.sourceOverall==='authoritative');const authText=await page.locator('#sourceHealthCard').innerText();assert.strictEqual(authoritative.overall,'authoritative');assert.ok(/PC-Manager\/KC-Core führend/.test(authText));assert.ok(/KC-Core\/Manager/.test(authText));assert.ok(/bereit/.test(authText));
  ...
  await page.waitForFunction(()=>document.querySelector('#sourceHealthCard')?.dataset.sourceOverall==='mixed');const mixedText=await page.locator('#sourceHealthCard').innerText();assert.strictEqual(mixed.overall,'mixed');assert.ok(/Gemischte Datenquellen/.test(mixedText));assert.ok(/teilweise/.test(mixedText));assert.ok(/noch nicht vollständig freigegeben/.test(mixedText));

  const html=await page.content();assert.ok(!/service_role/i.test(documentSafe(html)),'Datenquellenanzeige darf keine Service-Role-Bezeichnung/Secrets ausgeben');
  console.log('KC DP2 V0.19.42 Datenquellenstatus: führend / gemischt / lokal PASS');
  await browser.close();

...
function parseDate(v){
  ...
  if(typeof v==='number'&&Number.isFinite(v)){
    // Excel serial date, including the historical 1900 leap-year offset used by XLSX.
...
const templateHeaders=['datum','kann von','kann bis','wunsch von','wunsch bis','sperrzeit von','sperrzeit bis','sperrtag','nur wenn nötig','bemerkung'];
...
function entry(date,start,end,wishType,comment,rowNumber,sourceField){return {date,start,end,wishType,comment:clean(comment),rowNumber,sourceField};}
function expandRow(row,index=0){
  ...
  if(!date){
    const relevant=['Kann von','Kann bis','Wunsch von','Wunsch bis','Sperrzeit von','Sperrzeit bis','Sperrtag','Nur wenn nötig','Von','Bis','Start','Ende'].some(k=>clean(getField(row,[k]))!==''&&!['nein','no','false','0'].includes(norm(getField(row,[k]))));
  ...
  // Backward-compatible simple files: Datum + Von + Bis -> available.
  ...
  if(blockedDay){
    const start=day?.start??0,end=day?.end??24;entries.push(entry(date,start,end,'unavailable',remark||'Sperrtag',rowNumber,'Sperrtag'));
  ...
  if(canFrom!=null&&canTo!=null){if(canTo<=canFrom)issues.push({rowNumber,level:'error',text:'„Kann bis“ muss nach „Kann von“ liegen.'});else entries.push(entry(date,canFrom,canTo,onlyIfNeeded?'if_needed':'available',remark||(onlyIfNeeded?'Nur wenn nötig':''),rowNumber,onlyIfNeeded?'Nur wenn nötig':'Kann'));}
  ...
  if(wishFrom!=null&&wishTo!=null){
    ...
    else entries.push(entry(date,wishFrom,wishTo,'preferred',remark||'Bevorzugte Wunschzeit',rowNumber,'Wunsch'));
  }
  ...
  if(blockFrom!=null&&blockTo!=null){if(blockTo<=blockFrom)issues.push({rowNumber,level:'error',text:'„Sperrzeit bis“ muss nach „Sperrzeit von“ liegen.'});else entries.push(entry(date,blockFrom,blockTo,'unavailable',remark||'Sperrzeit',rowNumber,'Sperrzeit'));}
  if(!hasTemplateFields&&legacyFrom!=null&&legacyTo!=null){if(legacyTo>legacyFrom)entries.push(entry(date,legacyFrom,legacyTo,'available',remark,rowNumber,'Von/Bis'));else issues.push({rowNumber,level:'error',text:'„Bis“ muss nach „Von“ liegen.'});}

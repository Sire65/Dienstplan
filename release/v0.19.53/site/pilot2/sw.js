...
function trustedMessageSource(e){try{return !!e.source?.url&&new URL(e.source.url).origin===self.location.origin}catch(_){return false}}
self.addEventListener('message',e=>{if(!trustedMessageSource(e)||e.data?.type!=='KC_DP_PILOT2_CONTEXT')return;e.waitUntil(save(e.data.ctx||{}).then(()=>e.ports?.[0]?.postMessage({ok:true})))});

(function(){
  ...
  function ruleIssues(candidate,{includeSoft=true}={}){
    ...
    const h=dur(candidate),dayHours=dailyHours(candidate.personId,candidate.date,candidate.id)+h,eventTotal=eventHours(candidate.personId,candidate.id)+h;
    ...
    if(r.maxDailyHours!=null&&dayHours>Number(r.maxDailyHours)+1e-9)issues.push({level:'error',code:'max_daily',text:`Persönliche Tagesgrenze ${r.maxDailyHours} Std. würde überschritten (${dayHours.toFixed(1)} Std.).`});
    if(r.maxEventHours!=null&&eventTotal>Number(r.maxEventHours)+1e-9)issues.push({level:'error',code:'max_event',text:`Persönliche Veranstaltungsgrenze ${r.maxEventHours} Std. würde überschritten (${eventTotal.toFixed(1)} Std.).`});

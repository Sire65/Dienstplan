(function(){
 ...
 async function transportDiagnosis(url){
   ...
   try{const r=await fetch(url+'/auth/v1/settings?kc_dp_transport='+Date.now(),{method:'GET',mode:'no-cors',cache:'no-store'});d.projectNoCors=!!r;}
 ...
 K.supabaseConnection={version:'0.19.18',contract:'KC_DP_SUPABASE_SYNC_V1',state,configure,configureIfPossible,setAccessToken,restoreSession,persistSession:saveSession,sessionSnapshot:()=>session?JSON.parse(JSON.stringify(session)):null,clearSession,hasAccessToken:()=>!!session?.access_token,signInAnonymously,signInWithPassword,sendOtp,verifyOtp,requestPasswordReset,updatePassword,currentMembership,provisionMemberAccess,deactivateMemberAccess,provisionTestMember,removeTestMember,sendClientReport,signOut,refreshSession,ensureSession,test,testProject,probeDatabase,probeRls,probeRoundtrip,transportDiagnosis,provider,validateConfig};
 configureIfPossible();
})();

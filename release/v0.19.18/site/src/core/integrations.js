(function(){
 ...
 function normalize(cfg){
  ...
  const goodRef='iddudrxuihdodnvejxcp',knownBadRefs=['lddudrxuihdodnvejjxcp','lddudrxuihdodnvejxcp'];
  let migrated=false;
 ...
 K.integrations={version:'0.17.10',defaults:clone(defaults),snapshot(){return normalize(K.integrationConfig)},restore(cfg){K.integrationConfig=normalize(cfg);return this.snapshot();},update(section,patch){if(!defaults[section])throw new Error('Unbekannte Integration.');K.integrationConfig=normalize(K.integrationConfig);K.integrationConfig[section]={...K.integrationConfig[section],...clone(patch||{})};K.recordAudit?.('integration.config.update',{entity:'integration_config',entityId:section,after:{...K.integrationConfig[section],publishableKey:section==='supabase'&&K.integrationConfig[section].publishableKey?'***gesetzt***':undefined}});return clone(K.integrationConfig[section]);}};
})();

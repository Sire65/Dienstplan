(function(){
  ...
  const PROGRAM_NAME='KC DP2 · neu';
  const VERSION='0.19.55';
  const BUILD=Number(window.KC_DP_BUILD||85);
  const HEARTBEAT_PATH='/functions/v1/kicc-program-heartbeat';
  const FLOW_PATH='/functions/v1/kicc-program-flow';
  const INTERVAL_MS=30000;
  const CORE_REF='ptblnpiroqftcvlsrhac';
  const CORE_ORIGIN=`https://${CORE_REF}.supabase.co`;
  const nativeFetch=window.fetch.bind(window);
  let authorization=null,apikey=null,lastCredentialAt=null,lastHeartbeatAt=null,lastHeartbeatError=null,lastFlowAt=null,lastFlowError=null;
  let timer=null;
  const instanceId=(()=>{try{const key='kc_dp2_kicc_instance_v1';let id=sessionStorage.getItem(key);if(!id){id=`dp2-${crypto.randomUUID?.()||Math.random().toString(36).slice(2)}`;sessionStorage.setItem(key,id);}return id;}catch(_){return `dp2-${Date.now()}`;}})();

(function(){
  ...
  function installButton(){const old=document.getElementById('kcDiagnosticsAdminEntry');if(!old)return false;if(old.dataset.kcDiagWatchdog==='3')return true;const b=old.cloneNode(true);b.dataset.kcDiagWatchdog='3';b.style.touchAction='manipulation';b.onclick=null;old.replaceWith(b);b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();run()});log('button-wire','green','Ein einzelner Klick-Handler ist aktiv');return true}
  ...
  K.diagnosticsWatchdog={version:'0.19.55-diagwatch-3-immediate-recovery',run,installButton,snapshot,log,reset,checkpoint,complete,readActive,showRecoveredFreeze};

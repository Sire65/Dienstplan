(function(){
  ...
  function setMode(name,mode,{confirmation=''}={}){if(!allowedAdmin())throw new Error('Nur Admin darf den Versandmodus ändern.');if(!['test','live'].includes(mode))throw new Error('Ungültiger Versandmodus.');if(mode==='live'&&String(confirmation).trim().toUpperCase()!=='LIVE')throw new Error('Für LIVE-Versand muss LIVE ausdrücklich bestätigt werden.');const before=channel(name),now=new Date().toISOString(),who=K.currentUser?.personId||K.currentUser?.name||'admin';state[name]={mode,armed:mode==='live',changedAt:now,changedBy:who};save();K.recordAudit?.('notifications.channel.mode',{entity:'notification_channel_safety',entityId:name,before,after:channel(name)});K.sync?.enqueue?.({entity:'notification_channel_safety',operation:'update',payload:{channel:name,...channel(name)},baseVersion:null});window.dispatchEvent(new CustomEvent('kc-notification-safety-change',{detail:{channel:name,status:status(name)}}));return status(name)}

/* KC-DP2 stable PWA/update engine V1.
   ...
   './src/adapters/storage.js?v=0.19.37'
   ...
   './src/adapters/supabase-provider.js?v=0.19.37'
   ...
   './src/ui/role-ux.js?v=0.19.37'
   './src/core/update-manager.js?v=0.19.37'
   './src/ui/update-ui.js?v=0.19.37'
   './src/ui/app.js?v=0.19.37'
   './src/ui/app.js?v=0.17.10' legacy regression marker
   './templates/KC_DP2_Wunschzeiten_Vorlage_Weihnachtsmarkt_2026.xlsx'
*/
const ENGINE='kc-dp-update-engine-v1.5';
...
async function fetchWithTimeout(input,opt={},timeoutMs=NETWORK_TIMEOUT_MS){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeoutMs);try{return await fetch(input,{...opt,signal:opt.signal||controller.signal});}finally{clearTimeout(timer);}}
...
async function fetchManifest(){const r=await fetchWithTimeout(`${UPDATE_MANIFEST}?sw=${Date.now()}`,{cache:'no-store'});if(!r.ok)throw new Error(`Manifest HTTP ${r.status}`);const m=await r.json();if(!m?.version||!Array.isArray(m.files))throw new Error('Manifest unvollständig');return m;}
async function cacheRelease(m){const cacheName=m.cacheName||`kc-dp-release-${m.version}`,cache=await caches.open(cacheName),files=m.files.filter(x=>x.runtime!==false);for(const f of files){const source=new URL(f.downloadPath||f.path,self.registration.scope).toString(),target=new URL(f.installPath||f.path,self.registration.scope).toString(),hit=await cache.match(target,{ignoreSearch:true});if(hit)continue;const r=await fetchWithTimeout(source,{cache:'no-store'});if(!r.ok)throw new Error(`${f.installPath||f.path}: HTTP ${r.status}`);await cache.put(target,r.clone());}return cacheName;}
...
async function refreshForcedRuntime(meta){try{if(!meta?.activeCache)return meta;const m=await fetchManifest();if(String(m.version)!==String(meta.activeVersion))return meta;const forced=m.files.filter(x=>x.runtime!==false&&x.forceRefresh===true);if(!forced.length)return meta;const cache=await caches.open(meta.activeCache);for(const f of forced){const source=new URL(f.downloadPath||f.path,self.registration.scope),target=new URL(f.installPath||f.path,self.registration.scope).toString();source.searchParams.set('kc_sw_refresh',Date.now().toString());const r=await fetchWithTimeout(source.toString(),{cache:'no-store'});if(!r.ok)throw new Error(`${f.installPath||f.path}: HTTP ${r.status}`);const buffer=await r.arrayBuffer();if(Number(f.bytes||0)>0&&Math.abs(buffer.byteLength-Number(f.bytes))>4)throw new Error(`${f.installPath||f.path}: Dateigröße stimmt nicht`);if(f.sha256){const h=await sha256Hex(buffer);if(h.toLowerCase()!==String(f.sha256).toLowerCase())throw new Error(`${f.installPath||f.path}: SHA-256 stimmt nicht`);}const headers=new Headers(r.headers);headers.set('X-KC-DP-Release',String(m.version));headers.set('X-KC-DP-Forced-Refresh','1');await cache.put(target,new Response(buffer,{status:200,headers}));}meta={...meta,forcedRefreshAt:new Date().toISOString(),engine:ENGINE};await writeMeta(meta);}catch(_){}return meta;}

...
for(const file of ['src/core/model.js','src/core/auth.js','src/core/planning.js','src/core/staffing.js','src/core/breaks.js','src/core/planner-engine.js']){
  vm.runInContext(await readFile(path.join(root,file),'utf8'),context,{filename:file});
}
const K=context.window.KCDP;
assert.equal(K.VERSION,'0.19.55');

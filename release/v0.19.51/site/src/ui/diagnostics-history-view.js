...
function patchNote(host){
  ...
  if(note)note.innerHTML='Standardansicht: <b>nur offene Meldungen</b>. Behobene Einträge stehen ausschließlich unter <b>Historie</b>; Testmeldungen ausschließlich unter <b>Tests</b>.';
}
function decorate(host){
  ...
  tabs.className='kc-diag-view-toggle kc-diag-history-tabs';
  tabs.setAttribute('role','group');
  tabs.setAttribute('aria-label','Meldungsbereich');
  ...
  function choose(scope){
    tabs.querySelectorAll('[data-kc-diag-scope]').forEach(b=>{
      const active=b.dataset.kcDiagScope===scope;
...
scan();
K.diagnosticsHistoryView={version:'0.19.51-history4',decorate};
})();

(function(){
 ...
 const friendlyTitle=r=>{
  const code=String(r?.error_code||'');
  if(isTest(r))return 'Kontrollierte Testmeldung';
  return ({
   'window.error':'Fensterfehler',
   'unhandledrejection':'Technischer Hintergrundfehler',
   'pwa.service_worker.redundant':'PWA-Update-Hinweis'
  })[code]||code.replace(/[._-]+/g,' ').replace(/\b\w/g,c=>c.toUpperCase())||'Fehlermeldung';
 };
 const severityIcon=v=>['critical','error'].includes(String(v||'').toLowerCase())?'🔴':String(v||'').toLowerCase()==='warning'?'🟡':'🔵';
 const severityRank=v=>({critical:0,error:1,warning:2,info:3}[String(v||'').toLowerCase()]??4);
 const statusRank=v=>({new:0,reviewed:1,resolved:2,archived:3}[String(v||'').toLowerCase()]??4);
 ...
 async function open(){
  if(!allowed()||!K.diagnostics)return;
  ...
  function filtered(){
   const f=host.querySelector('#kcDiagFilter').value,q=host.querySelector('#kcDiagSearch').value.trim().toLowerCase();
   return sortRows(allRows.filter(r=>{
    const test=isTest(r);
    const ok=f==='all_with_tests'||(f==='all'?!test:f==='tests'?test:f==='open'?!test&&isOpen(r):f==='critical'?!test&&r.severity==='critical'&&isOpen(r):f==='resolved'?!test&&r.status==='resolved':!test&&r.status===f);
    const hay=[r.member_name,r.person_id,r.device_id,r.error_code,friendlyTitle(r),r.message,r.app_version,r.platform,r.browser].some(v=>String(v||'').toLowerCase().includes(q));
    return ok&&(!q||hay);
   }))
  }
  function summary(){
   const normal=allRows.filter(r=>!isTest(r)),openRows=normal.filter(isOpen),critical=openRows.filter(r=>r.severity==='critical'),resolved=normal.filter(r=>r.status==='resolved'),tests=allRows.filter(isTest);
   host.querySelector('#kcDiagSummary').innerHTML=`<div class="kc-diag-stats"><div><b>${openRows.length}</b><span>offen</span></div><div><b>${critical.length}</b><span>kritisch</span></div><div><b>${new Set(openRows.map(r=>r.device_id).filter(Boolean)).size}</b><span>Geräte</span></div><div><b>${new Set(openRows.map(r=>r.person_id).filter(Boolean)).size}</b><span>Mitglieder</span></div></div><div class="kc-diag-housekeeping">Standardansicht: <b>${resolved.length}</b> behobene und <b>${tests.length}</b> Testmeldungen ausgeblendet. Über den Filter können sie jederzeit angezeigt werden.</div>`
  }
  ...
  function actionHtml(r){
   if(['resolved','archived'].includes(r.status))return '<span class="kc-diag-no-action">Keine Aktion nötig</span>';
   return `${r.status==='new'?`<button data-id="${esc(r.id)}" data-status="reviewed">Geprüft</button>`:''}<button data-id="${esc(r.id)}" data-status="resolved" class="primary">${isTest(r)?'Test erledigt':'Behoben'}</button>`;
  }
  function wireActions(thost){thost.querySelectorAll('[data-status]').forEach(b=>b.onclick=()=>setOne(b.dataset.id,b.dataset.status))}
  function cards(rows,thost){
   thost.className='kc-diag-card-list';
   thost.innerHTML=rows.length?rows.map(r=>`<article class="kc-diag-mobile-card ${esc(r.severity)} ${isTest(r)?'test':''} ${esc(r.status)}"><div class="kc-diag-mobile-top"><div class="kc-diag-badges"><span class="kc-diag-severity">${severityIcon(r.severity)} ${esc(severityLabel(r.severity))}</span>${isTest(r)?'<span class="kc-diag-test-badge">TEST</span>':''}</div><span class="kc-diag-status">${esc(statusLabel(r.status))}</span></div><h3>${esc(friendlyTitle(r))}</h3><div class="kc-diag-code">Technischer Code: ${esc(r.error_code||'–')}</div><p class="kc-diag-message">${esc(r.message||'')}</p><div class="kc-diag-mobile-meta"><span><b>Version</b><em>${esc(r.app_version||'–')}</em></span><span><b>Anzahl</b><em>${Number(r.occurrence_count||1)}×</em></span><span><b>Zuletzt</b><em>${fmt(r.last_seen_at)}</em></span></div><p class="kc-diag-device"><b>${esc(r.member_name||r.person_id||'Unbekannt')}</b><br><span>${esc(r.device_id||'–')}</span> · ${esc(r.platform||'')} · ${r.online===false?'offline':'online'}</p><details><summary>Technische Details</summary><p><b>Status:</b> ${esc(statusLabel(r.status))}<br><b>Code:</b> ${esc(r.error_code||'–')}<br><b>Erstmals:</b> ${fmt(r.first_seen_at)}<br><b>Quelle:</b> ${esc(r.source||'–')}<br><b>Route:</b> ${esc(r.route||'–')}<br><b>Browser:</b> ${esc(r.browser||'–')}</p><pre>${esc(r.stack||'Kein Stacktrace')}</pre></details><div class="kc-diag-mobile-actions">${actionHtml(r)}</div></article>`).join(''):'<p class="kc-diag-empty">Keine Meldungen für diesen Filter.</p>';
   wireActions(thost)
  }
  function render(){
   summary();
   updateViewToggle();
   const rows=filtered(),thost=host.querySelector('#kcDiagTable');
   thost.className='';
   if(effectiveView()==='cards'){cards(rows,thost);return}
   const columns=[
    {key:'severity',label:'Stufe',render:r=>`${severityIcon(r.severity)} ${esc(severityLabel(r.severity))}${isTest(r)?' · TEST':''}`},
    {key:'status',label:'Status',render:r=>esc(statusLabel(r.status))},
    {key:'member_name',label:'Mitglied'},
    {key:'message',label:'Meldung',render:r=>`<b>${esc(friendlyTitle(r))}</b><br>${esc(r.message||'')}<br><small>${esc(r.error_code||'–')}</small>`},
    {key:'app_version',label:'Version'},
    {key:'occurrence_count',label:'Anzahl',render:r=>`${Number(r.occurrence_count||1)}×`},
    {key:'last_seen_at',label:'Zuletzt',render:r=>fmt(r.last_seen_at)},
    {key:'device_id',label:'Gerät / Browser',render:r=>`${esc(r.platform||'')}<br><small>${esc(r.device_id||'–')}</small>`},
    {key:'actions',label:'Aktion',render:r=>`<div class="kc-diag-table-actions">${actionHtml(r)}</div>`}
   ];
   if(K.tableCore){K.tableCore.create(thost,{rows,columns});wireActions(thost)}else cards(rows,thost)
  }
  async function load(){
   if(loading)return;
   loading=true;
   try{if(!navigator.onLine)throw new Error('Dieses Gerät ist offline.');allRows=await withTimeout(K.diagnostics.adminList(500),LOAD_TIMEOUT_MS,'Supabase-Diagnose');render()}
   catch(e){host.querySelector('#kcDiagTable').innerHTML=`<div class="kc-diag-load-error"><b>Diagnose konnte nicht geladen werden.</b><p>${esc(e.message)}</p><button id="kcDiagRetry">Erneut versuchen</button></div>`;host.querySelector('#kcDiagRetry')?.addEventListener('click',load)}
 ...
 function inject(){
  if(!allowed()||document.getElementById('kcDiagnosticsAdminEntry'))return;

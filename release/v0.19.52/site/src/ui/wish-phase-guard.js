...
function protectWishInputs(root=document){const s=state(),locked=s.phase!=='open';const selectors=['[data-wish-edit]','[data-wish-delete]','[data-copywish]','#wsCopy','#photoBtn','[data-action="wish-import"]','[data-action="wish-edit"]'];
  root.querySelectorAll?.(selectors.join(',')).forEach(el=>{if(!locked)return;el.disabled=true;el.setAttribute('aria-disabled','true');el.title=label();el.classList.add('kc-wish-locked-control');});
}
...
K.wishPhaseGuard={version:'0.19.51-safe-1',state,label,help,assertOpen,setDeadline,closePhase,reopenPhase,renderStatus};
})();

...
function enhanceMemberEntry(root=document){const s=state(),btn=root.querySelector?.('#uxStartTimes');if(!btn)return;const card=btn.closest?.('.ux-card');if(s.phase==='open'){
  ...
  // Wunschplan bleibt bewusst lesbar, ist aber optisch eindeutig nicht mehr bearbeitbar.
  btn.classList.add('kc-wish-readonly-entry');btn.textContent='Nur ansehen';btn.title=label();btn.setAttribute('data-phase-note',label());card?.classList.add('kc-wish-readonly-card');
...
function protectWishInputs(root=document){const s=state(),locked=s.phase!=='open';const selectors=['[data-wish-edit]','[data-wish-delete]','[data-copywish]','#wsCopy','#photoBtn','[data-action="wish-import"]','[data-action="wish-edit"]'];
  root.querySelectorAll?.(selectors.join(',')).forEach(el=>{if(!locked)return;el.disabled=true;el.setAttribute('aria-disabled','true');el.title=label();el.classList.add('kc-wish-locked-control');});enhanceMemberEntry(root);
}
...
function normalizeTabs(){document.querySelectorAll('#layerTabs [data-layer]').forEach(b=>{const map={wish:'Wunsch',planned:'Soll',actual:'Ist',compare:'Vergleich'};if(map[b.dataset.layer])b.textContent=map[b.dataset.layer];});}
...
K.wishPhaseGuard={version:'0.19.52-safe-2',state,label,help,assertOpen,setDeadline,closePhase,reopenPhase,renderStatus};
})();

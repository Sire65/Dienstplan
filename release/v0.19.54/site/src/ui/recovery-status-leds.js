(function(){
  ...
  function ensureStyle(){
    ...
    const s=document.createElement('style');s.id='kcRecoveryLedStyle';s.textContent=`
      ...
      @media(max-width:560px){.kc-recovery-db{gap:4px;margin-right:2px}.kc-recovery-dbcol{min-width:38px;padding:2px 4px}.kc-recovery-dblabel{font-size:8px}}
    `;document.head.appendChild(s);
  }

  function ensureRoleLeds(){
    const bar=document.querySelector('.ux-topbar');
    if(!bar||bar.querySelector('#kcRecoveryDbBlock'))return;
    ...
    const user=bar.querySelector('.ux-userchip');
    if(user)bar.insertBefore(box,user);else bar.appendChild(box);
  }
  ...
  ensureStyle();paint();
  K.recoveryStatusLeds={version:'0.20.0-recovery-1',paint,flash,status:()=>({idb:clsFor('idb'),supabase:clsFor('sup')})};
})();

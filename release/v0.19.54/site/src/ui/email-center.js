...
async function load(root){const status=root.querySelector('[data-mail-status]'),client=supa();if(!client?.from){status.textContent='Supabase noch nicht verbunden.';return}
 try{
  const [{data:providers,error:pErr},{data:inbox,error:iErr}]=await Promise.all([
   client.from('kc_dp_mail_provider_settings').select('id,provider_key,display_name,enabled,priority,send_enabled,receive_enabled,free_tier_note,last_health_status,last_health_at,last_error').order('priority'),
   client.from('kc_dp_inbox_messages').select('id,received_at,from_address,subject,status,person_id,person_match_method,person_match_confidence,document_id,quarantine_reason').order('received_at',{ascending:false}).limit(100)
  ...
  tc.create(root.querySelector('[data-provider-table]'),{selectable:false,filterPlaceholder:'Provider filtern …',rows:(providers||[]).map(x=>({...x,id:String(x.id)})),columns:[
   {key:'priority',label:'Prio'},{key:'display_name',label:'Anbieter'},{key:'send_enabled',label:'Senden',render:r=>r.send_enabled?'✓':'–'},{key:'receive_enabled',label:'Empfang',render:r=>r.receive_enabled?'✓':'–'},{key:'free_tier_note',label:'Kostenlos/Limits'},{key:'last_health_at',label:'Letzter Test',render:r=>esc(r.last_health_at?new Date(r.last_health_at).toLocaleString('de-DE'):'–')},{key:'last_health_status',label:'Status',render:r=>esc(r.last_error||r.last_health_status||'unbekannt')}
  ]});
  tc.create(root.querySelector('[data-inbox-table]'),{selectable:true,filterPlaceholder:'Posteingang filtern …',rows:(inbox||[]).map(x=>({...x,id:String(x.id)})),columns:[
   {key:'received_at',label:'Eingang',render:r=>esc(r.received_at?new Date(r.received_at).toLocaleString('de-DE'):'–')},{key:'from_address',label:'Absender'},{key:'subject',label:'Betreff'},{key:'status',label:'Status'},{key:'person_id',label:'Person'},{key:'person_match_confidence',label:'Sicherheit',render:r=>esc(r.person_match_confidence==null?'–':`${Math.round(Number(r.person_match_confidence)*100)} %`)},{key:'person_match_method',label:'Zuordnung'},{key:'quarantine_reason',label:'Hinweis'}
  ]});

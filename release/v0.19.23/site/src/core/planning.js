(function(){
  ...
  K.specialHoursForPerson=function(personId,date){
    return K.shifts.filter(s=>s.personId===personId&&s.date===date&&s.layer==='planned'&&s.zone==='special'&&plannedActive(s)).reduce((a,s)=>a+durationHours(s),0);
  };
  ...
  K.coverageAt=function(day,time,override=null){
    ...
    const special=allActive.filter(s=>s.zone==='special');
    const active=allActive.filter(s=>s.zone!=='special');
  ...
  K.evaluateDay=function(day,override=null){
    ...
    const specialHours=activeBase.filter(s=>s.zone==='special').reduce((a,s)=>a+durationHours(s),0);

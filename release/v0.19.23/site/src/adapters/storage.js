  ...
  K.storage={
    ...
    async init(){
      ...
      return new Promise((resolve,reject)=>{
        ...
        req.onsuccess=()=>{this.db=req.result;resolve(true)};req.onerror=()=>reject(req.error);
      });
    },
    ...
    async getMany(keys,{onProgress,migrate=true}={}){
      ...
      const started=performance.now(),raw=await new Promise((resolve,reject)=>{const out=new Array(list.length),tx=this.db.transaction(STORE,'readonly'),os=tx.objectStore(STORE);list.forEach((key,i)=>{const q=os.get(key);q.onsuccess=()=>out[i]=q.result;q.onerror=()=>reject(q.error)});tx.oncomplete=()=>resolve(out);tx.onerror=()=>reject(tx.error)});

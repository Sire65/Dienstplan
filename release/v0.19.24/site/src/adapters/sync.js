(function(){
  ...
  function enqueue(input){const op=makeOp(input);K.syncOutbox.push(op);emit('queue',{operation:op});return op;}
  ...
  async function flush(){
    ...
    K.recordAudit?.('sync.flush',{entity:'sync',after:{sent,conflicts,failed,pending:K.syncOutbox.length}});return {sent,conflicts,failed,pending:K.syncOutbox.length};
  }
  function resolveConflict(id,choice){
    ...
    if(choice==='local')enqueue({entity:c.entity,operation:'force_update',payload:c.local,baseVersion:c.remote?.version??null});
  ...
  K.sync={
    ...
    on(fn){if(typeof fn==='function')listeners.add(fn);return()=>listeners.delete(fn);},enqueue,healthCheck,flush,pull,syncBoth,resolveConflict,

(function(){
  ...
  async function edge(action,payload={}){const conn=K.supabaseConnection,c=conn?.validateConfig?.();if(!c)throw new Error('Für Push fehlt die Supabase-Konfiguration.');let token=conn?.sessionSnapshot?.()?.access_token;if(!token&&K.storage?.unlocked){try{const saved=await K.storage.get('supabaseSession');if(saved)conn?.restoreSession?.(saved);}catch(_){/* gespeicherte Sitzung optional */}}try{await conn?.ensureSession?.();}catch(_){/* unten klare Meldung */}token=conn?.sessionSnapshot?.()?.access_token;if(!token)throw new Error('Für Push ist eine gültige Supabase-Anmeldung erforderlich. Bitte einmal neu anmelden.');const r=await fetch(`${c.url}/functions/v1/${state.edgeFunction}`,{method:action==='config'?'GET':'POST',headers:{apikey:c.publishableKey,Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:action==='config'?undefined:JSON.stringify({action,orgId:c.orgId,projectId:c.projectId,...payload})}),data=await r.json();if(!r.ok)throw new Error(data.error||`Push HTTP ${r.status}`);return data;}

(function(){
 ...
 async function request(payload){const c=config(),fn=direct||hostFn();state.mode=c.mode||'host';state.status='connecting';try{let out;if(c.mode==='url')out=await urlRequest(payload);else if(fn)out=await fn({...payload,contract:'KC_PC_MANAGER_DP_BRIDGE_V1'});else out=await postMessageRequest(payload);state.status='ready';state.lastError=null;state.lastSource=out?.meta?.source||c.mode||'host';return out;}catch(e){state.status='error';state.lastError=e.message;throw e;}}

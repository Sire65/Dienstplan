...
VERSION=sys.argv[1] if len(sys.argv)>1 else '0.19.15'
OUT=ROOT/'build/site'
...
if VERSION in ['0.19.14','0.19.15']:apply('0.19.14')
if VERSION=='0.19.15':apply('0.19.15')
elif VERSION not in ['0.19.13','0.19.14']:raise SystemExit(f'unsupported release {VERSION}')
...
notes={
'0.19.13':['Performance-Core: PBKDF2 einmal pro Sitzung statt pro IndexedDB-Zugriff','Bulk-IndexedDB-Transaktionen für Laden und Speichern','Einmalige sichere Migration bestehender lokaler Daten','Fortschrittsanzeige mit Prozent, Laufzeit und Restzeit für längere Vorgänge'],
'0.19.14':['Supabase sauber getrennt: KC-DP2 verwendet ein eigenes Dienstplan-Projekt','Bestehende Academy-Konfiguration wird automatisch auf das neue KC-DP-Projekt migriert','Publishable Key und Projektprofil sind fest auf den freigegebenen KC-DP-Stand gebunden','Sync-, Verschlüsselungs- und Rollback-Mechanismen bleiben unverändert'],
'0.19.15':['Lokales Speichern schreibt nur tatsächlich geänderte Datenbereiche statt pauschal 34 Bereiche','Startladen liest nur die angeforderten IndexedDB-Schlüssel in einer gemeinsamen Transaktion','Login und Start zeigen Fortschritt, Laufzeit und geschätzte Restzeit','Performance-Diagnose zeigt Dauer und Anzahl der zuletzt gespeicherten Bereiche']
}[VERSION]
manifest={'schema':'KC_DP_UPDATE_MANIFEST_V1','app':'KC-DP2','version':VERSION,'cacheName':f'kc-dp-release-{VERSION}','releaseNotes':notes,'generatedAt':'2026-08-13T07:10:00+02:00','totalRuntimeBytes':total,'files':files}
